...
//======================
	...
	// UI's vars
		...
		static private var mainMenuButton:Sprite;
		static private var MenuOptionButton1:Sprite;
		static public var MenuOptionButton1Text:TextField;
		static private var drawingPlain:Sprite;
		static public var drawingPlainText:TextField;
		static private var menuLine:Sprite;
		static private var menuState:String = "00";
	...
	// add new vars
		var addNewX1:Number = 0;
		var addNewY1:Number = 0;
		var addNewX2:Number = 0;
		var addNewY2:Number = 0;
		
	// blueprint
		var blueprint:Sprite = new Sprite();
...
//======================
	...
	// UI Elements init.
		...
		// Menu button
			...
			mainMenuButton.graphics.beginFill(0x000000, 0.1);
			mainMenuButton.graphics.lineStyle(5, 0xEEEEEE, 1, false, "none");
			mainMenuButton.graphics.drawCircle(0, 0, 23);
			mainMenuButton.graphics.endFill();
			mainMenuButton.graphics.beginFill(0x000000, 0.1);
			mainMenuButton.graphics.drawCircle(0, -9.5, 7);
			mainMenuButton.graphics.endFill();
			mainMenuButton.graphics.beginFill(0x000000, 0.1);
			mainMenuButton.graphics.drawCircle(-8.5, 4, 7);
			mainMenuButton.graphics.endFill();
			mainMenuButton.graphics.beginFill(0x000000, 0.1);
			mainMenuButton.graphics.drawCircle(7.5, 4, 7);
			mainMenuButton.graphics.endFill();
			mainMenuButton.addEventListener(MouseEvent.MOUSE_DOWN, ToggleMenu);
			mainMenuButton.addEventListener(TouchEvent.MOUSE_DOWN, ToggleMenu);
			mainMenuButton.addEventListener(MouseEvent.MOUSE_OUT, MainMenuButtonInactive);
			mainMenuButton.addEventListener(TouchEvent.MOUSE_OUT, MainMenuButtonInactive);
			mainMenuButton.addEventListener(MouseEvent.MOUSE_OVER, MainMenuButtonActive);
			mainMenuButton.addEventListener(TouchEvent.MOUSE_OVER, MainMenuButtonActive);
			mainMenuButton.x = 38;
			mainMenuButton.y = 38;
			stage.addChildAt(mainMenuButton, stage.numChildren-1);								
			mainMenuButton.alpha = 0.25;		 

			menuLine = new Sprite();
			menuLine.graphics.lineStyle(5, 0xEEEEEE, 1, false, "none");
			menuLine.graphics.drawRect(0, 0, 0, 1);
			menuLine.x = 11;
			menuLine.y = 39;
			menuLine.alpha = 0;
			stage.addChildAt(menuLine, stage.numChildren-1);

			MenuOptionButton1 = new Sprite();
			MenuOptionButton1.graphics.lineStyle(5, 0xEEEEEE, 1, false, "none");
			MenuOptionButton1.graphics.beginFill(0x00FF00, 0.3);
			MenuOptionButton1.graphics.endFill();
			MenuOptionButton1.graphics.drawCircle(25, 25, 25);
			MenuOptionButton1.graphics.beginFill(0x666666, 0.8);
			MenuOptionButton1.graphics.drawRect(12.5, 12.5, 25, 25);
			MenuOptionButton1.graphics.endFill();
			MenuOptionButton1.x = -1000; // 11
			MenuOptionButton1.y = 75;
			MenuOptionButton1.alpha = 0;
			stage.addChildAt(MenuOptionButton1, stage.numChildren-1);
			MenuOptionButton1.addEventListener(MouseEvent.MOUSE_DOWN, MenuOptionButton1Start);
			MenuOptionButton1.addEventListener(TouchEvent.MOUSE_DOWN, MenuOptionButton1Start);
			MenuOptionButton1.addEventListener(MouseEvent.MOUSE_OVER, MenuOptionButton1Active);
			MenuOptionButton1.addEventListener(TouchEvent.MOUSE_OVER, MenuOptionButton1Active);
			MenuOptionButton1.addEventListener(MouseEvent.MOUSE_OUT, MenuOptionButton1Stop);
			MenuOptionButton1.addEventListener(TouchEvent.MOUSE_OUT, MenuOptionButton1Stop);
			
		// plain - needed to invoke drawiibg - to be checked if no other way
			drawingPlain = new Sprite();
			drawingPlain.graphics.beginFill(0xFFFFFF, 1);
			drawingPlain.graphics.drawRect(0, 0, stage.stageWidth, stage.stageHeight);
			drawingPlain.graphics.endFill();
			drawingPlain.alpha = 0;
			drawingPlain.name = "drawingPlain";
			drawingPlain.x = -5000;
			stage.addChildAt(drawingPlain, 1);

			MainMenuButtonActive("init");
			MainMenuButtonInactive("init");
...
//======================
// MenuOptionButton1Active
//======================
		public function MenuOptionButton1Active(e:String):void 
		{
			Tweener.addTween(MenuOptionButton1, {_Glow_alpha:0.6, _Glow_color:0x00FF00, _Glow_quality:3, _Glow_strength:2, _Glow_blurX:20, _Glow_blurY:20, time:1, transition:"easeoutelastic"} );
			menuState = "01";
		}
...
//======================
		public function MainMenuButtonActive(e:String):void 
		{
			Tweener.addTween(mainMenuButton, {alpha:1, time:0.5, transition:"easeIn"} );
			Tweener.addTween(mainMenuButton, {_Glow_alpha:0.6, _Glow_color:0x00FF00, _Glow_quality:3, _Glow_strength:2, _Glow_blurX:20, _Glow_blurY:20, time:1, transition:"easeoutelastic"} );
		}

//======================
// MainMenuButtonInactive
//======================
		public function MainMenuButtonInactive(e:String):void 
		{
			Tweener.addTween(mainMenuButton, {_Glow_alpha:0, time:1, transition:"easeoutelastic"} );
			if(menuState == "00")
			{
				Tweener.addTween(mainMenuButton, {alpha:0.25, time:0.5, transition:"easeOut"} );
			}
		}
		
//======================================
// ToggleMenu - sets to predefined state
//======================================
		public function ToggleMenu(e:String):void 
		{
			switch(menuState)
			{
				// All menu off
					case "00":
						trace("Menu 00: " + e);
						Tweener.addTween(mainMenuButton, {scaleX:1.2, scaleY:1.2, rotation:-120, time:2, transition:"easeoutelastic"});
						MainMenuButtonActive("00");
						Tweener.addTween(MenuOptionButton1, {alpha:1, time:0.5, transition:"easeOutQuint"});
						Tweener.addTween(menuLine, {x:11, scaleY:55, alpha:1, time:0.5, transition:"easeInQuint"});
						MenuOptionButton1.x = 11;
						menuState = "10";
						break;

				// Main menu button on the rest off
					case "10":
						trace("Menu 10: " + e);
						Tweener.addTween(mainMenuButton, {scaleX:1, scaleY:1, rotation:120, time:2, transition:"easeoutelastic", _Glow_alpha:1});			
						MainMenuButtonInactive("10");
						Tweener.addTween(MenuOptionButton1, {alpha:0, time:0.5, transition:"easeOutQuint"});
						Tweener.addTween(menuLine, {x:11, scaleY:0.55, alpha:0, time:0.4, transition:"easeOutQuint"});
						menuState = "00";
						break;


				// Main menu button on some of the rest on
					case "01":
						trace("Menu 01: " + e);
						Tweener.addTween(mainMenuButton, {scaleX:1, scaleY:1, rotation:120, time:2, transition:"easeoutelastic", _Glow_alpha:1});			
						MainMenuButtonInactive("01");
						Tweener.addTween(MenuOptionButton1, {alpha:0, time:0.5, transition:"easeOutQuint"});
						Tweener.addTween(menuLine, {x:11, scaleY:0.55, alpha:0, time:0.4, transition:"easeOutQuint"});
						MenuOptionButton1.x = -1000;
						menuState = "00";
						break;
						}
		}
...
//======================
// MenuOptionButton1Start
//======================
		public function MenuOptionButton1Start(e:Event):void 
		{
//			stage.addEventListener(MouseEvent.MOUSE_DOWN, AddNewDownHandler);
			stage.addEventListener(TouchEvent.MOUSE_DOWN, AddNewDownHandler);
//			stage.addEventListener(MouseEvent.MOUSE_UP, AddNewUpHandler);
			stage.addEventListener(TouchEvent.MOUSE_UP, AddNewUpHandler);
//			stage.addEventListener(MouseEvent.MOUSE_MOVE, AddNewMoveHandler);
			stage.addEventListener(TouchEvent.MOUSE_MOVE, AddNewMoveHandler);

		// Blueaprint drawing support plain
			drawingPlain.x = 0;
...
//======================
// MenuOptionButton1Stop
//======================
		public function MenuOptionButton1Stop(e:Event):void 
		{
			drawingPlain.x = -5000;
//			stage.removeEventListener(MouseEvent.MOUSE_DOWN, AddNewDownHandler);
			stage.removeEventListener(TouchEvent.MOUSE_DOWN, AddNewDownHandler);
//			stage.removeEventListener(MouseEvent.MOUSE_UP, AddNewUpHandler);
			stage.removeEventListener(TouchEvent.MOUSE_UP, AddNewUpHandler);
//			stage.removeEventListener(MouseEvent.MOUSE_MOVE, AddNewMoveHandler);
			stage.removeEventListener(TouchEvent.MOUSE_MOVE, AddNewMoveHandler);
			Tweener.addTween(MenuOptionButton1, {_Glow_alpha:0, time:0.5, transition:"easeOut"} );
			ToggleMenu("");
			MainMenuButtonInactive("");
		}
...
// AddNewDownHandler
//======================
			public function AddNewDownHandler(e:TouchEvent):void 
			{
//				addNewX1 = stage.mouseX;
//				addNewY1 = stage.mouseY;			
				addNewX1 = e.localX;
				addNewY1 = e.localY;			
			}
...
// AddNewUpHandler
//======================
			public function AddNewUpHandler(e:TouchEvent):void
			{
//				addNewX2 = stage.mouseX;
//				addNewY2 = stage.mouseY;
				addNewX2 = e.localX;
				addNewY2 = e.localY;
				
		// Building Shape and Body 
				var shapeW = Math.abs(addNewX1-addNewX2); 	// witdth (in px) of our new shape
				var shapeH = Math.abs(addNewY1-addNewY2);	// height (in px) of our new shape
				var boxDef:b2PolygonDef = new b2PolygonDef();
				boxDef.SetAsBox(shapeW * 0.5 / m_physScale, shapeH * 0.5 / m_physScale);
				boxDef.friction = 0.3; 			// 0 .. 1
				boxDef.density = 1; 			// only needed for dynamic bodies
				boxDef.restitution = 0.5;		// only needed for dynamic bodies
				var bodyDef:b2BodyDef = new b2BodyDef();
				bodyDef.position.Set((addNewX1+(addNewX2-addNewX1)*0.5) / m_physScale, (addNewY1+(addNewY2-addNewY1)*0.5) / m_physScale);
				var body:b2Body = m_world.CreateBody(bodyDef);
				body.CreateShape(boxDef);
				body.SetMassFromShapes();
			
		// clearing blueprint graphics after B2D shape has been drawn
				blueprint.graphics.clear();	
				
		// puts walls under newly created objects
			setWalls(stage.stageWidth, stage.stageHeight);
			}
...
// AddNewMoveHandler
//======================
			public function AddNewMoveHandler(e:TouchEvent):void
			{
				addChild(blueprint);
				
		// clearing (old) blueprint graphics while mouse moves
				blueprint.graphics.clear();		
				blueprint.graphics.lineStyle(1, 0x23FF23, 0.5);
				blueprint.graphics.beginFill(0x23FF23, 0.2);
					var blueprintX1 = addNewX1;
					var blueprintY1 = addNewY1;
					var blueprintX2 = e.localX;
					var blueprintY2 = e.localY;
					if(blueprintX1 > blueprintX2){blueprintX2 = blueprintX1; blueprintX1 = e.localX;}
					if(blueprintY1 > blueprintY2){blueprintY2 = blueprintY1; blueprintY1 = e.localY;}
					blueprint.graphics.drawRect(blueprintX1, blueprintY1, Math.abs(blueprintX2-blueprintX1), Math.abs(blueprintY2-blueprintY1));
			}
...
//======================
		public function InputDrag():void
		{
			for(var i:int=0; i<blobs.length; i++)
			{
				if(blobs[i].m_Joint)
				{				
					var xworld:Number = blobs[i].x/m_physScale, yworld:Number = blobs[i].y/m_physScale;
